...
def Get_Energy(FileName, CPU, Z, Charge, Method, BasisSet, Scales):
 ...
 #   bash_lines = ['#$ -S /bin/csh\n', '#$ -cwd\n', '#$ -j y\n','#$ -l h_rt=00:01:00,test=true\n', '#$ -pe open mp 4\n', '#$ -l h_rt=00:01:00,test=true mpirun myapplication\n\n']
 #   bash_name  =  FileName + '.sh'
 #   job_call   =  subprocess.call('g09 < '+ FileName + '.gjf > ' + FileName + '.out', shell=True)
    #job_call   =  'g09 < '+ FileName + '.gjf > ' + FileName + '.out'
 #   qsub_call  =  'qsub FileName + .sh' 
